...
try {
    ...
    $stmt = $pdo->prepare("SELECT slot_id FROM parking_slots WHERE slot_number = ?");
    ...
    // Insert new slot
    $stmt = $pdo->prepare("
        INSERT INTO parking_slots (slot_number, status, last_updated)
        VALUES (?, ?, NOW())
    ");
    $stmt->execute([$slotNumber, $status]);
    ...
    $stmt = $pdo->prepare("
        ...
        VALUES (?, ?, NOW())
    ");
    $stmt->execute([$_SESSION['user_id'], "Added new parking slot: {$slotNumber}"]);
