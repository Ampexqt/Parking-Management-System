...
try {
    ...
    $stmt = $pdo->prepare("
        UPDATE payments 
        SET payment_status = 'declined', approved_by = ?, payment_date = NOW()
        WHERE payment_id = ?
    ");
    $stmt->execute([$adminId, $paymentId]);
    
    // Update parking session to cancelled
    $stmt = $pdo->prepare("
    ...
    $stmt = $pdo->prepare("
        ...
        VALUES (?, ?, NOW())
    ");
    $stmt->execute([$adminId, "Declined payment #{$paymentId} for amount $" . $payment['amount']]);
